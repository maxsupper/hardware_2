...
def main(product: str = "FL-25-E-MR203"):
    ...
    import json
    # L1 工具单元
    nets = json.loads((P / "PH-2_netlist" / "global_nets.json").read_text(encoding="utf-8"))
    comps = json.loads((P / "PH-2_netlist" / "global_components.json").read_text(encoding="utf-8"))
    add("L1-edn_parse(真实)", f"全局元件={len(comps)} 网络={len(nets)}",
        len(comps) > 500, "global_components.json/global_nets.json")
    add("L1-gate(G1)", "G1 状态", gv.validate_manual_bom(P / "PH-1_manual").status.value == "PASS",
        f"gates/G1.json -> {gv.main_gate_cli(P, 'G1')}")
    add("L1-gate(G2)", "G2 状态", gv.validate_netlist(P / "PH-2_netlist").status.value == "PASS",
    ...
    add("L2-负向-fault注入(G4缺报告)", "report.json 缺失 → G4 应 FAIL",
        gv.validate_report(fdir).status.value == "FAIL", "PH-4_report/report.json")
    ...
    add("L3-产出齐全", f"{len(files)} 个产物文件", any("PH-2_netlist" in f for f in files)
        and any("PH-4_report" in f for f in files) and any("gates" in f for f in files),
