...
AGENT_CONTENT = {
    "hw_review": {
        ...
        "goal": ("驱动 PH-0..PH-7 全流程：每个 Gate 读取 gates/*.json 验证 status，"
                 "PASS 放行 / FAIL 硬阻断并退回对应 subagent；维护阶段 todo；"
                 "最终产出 final_report + g6_closure"),
    ...
    "hw_search": {
        "type": "agent",
        "role": "芯片数据手册检索专员",
        "goal": ("递归搜索本地手册库（md/pdf/txt/xlsx 全格式），为网表中每颗功能 IC 匹配数据手册；"
                 "对 FOUND_PARTIAL/MISSING 的 IC 至少做 ≥3 次有效检索尝试"
                 "（本地+ Tavily≥2 策略）；匹配成功下载存档到 storge/datasheet；"
                 "产出 g0_sources.json + g0_ic.msg 及 Step0.6 汇总表"),
    ...
    "hw_prep": {
        "type": "agent",
        "role": "网表解析工程师（EDN/EDIF 确定性解析为主，LLM 兜底）",
        "goal": ("将多个 EDN 网表逐文件解析→全局合并→构造完整信号链，产出 "
                 "prep/components.json、global_nets.json、signal_chains.json、"
                 "connectors.json 及 by_ic/*.json；只写事实、零电气判断，100% 覆盖"),
    ...
    "hw_analyze": {
        "type": "agent",
        "role": "硬件原理图设计验证专家（深度分析）",
        "goal": ("对每颗功能 IC 执行全维度检查：引脚核对、VCCIO 电平匹配、供电网络追踪、"
                 "DDR/外设/连接器/浮空检测；产出 evidence/{name}_evidence.json 与 "
                 "_summary.json(≤5KB)；每个结论带五级判定（CRITICAL/WARNING/OK/INFERRED/UNVERIFIED）"
                 "+ 手册原文页码或 EDN 行号证据，逐项独立、禁止概括"),
        "backstory": ("十几年原理图审查经验，把'网名是标签而非物理事实'刻进骨子里——"
                      "一切判定从物理连接追踪与手册原文出发，绝不用规则文档二手描述或经验"
...
def main() -> None:
    ...
    for name, content in AGENT_CONTENT.items():
        ...
        bootstrap = ",".join(cfg.get("bootstrap", [])) or "-"
        yaml_txt = (
