...
NEW_RULES = {
    ...
    "PF-005": ("规则束预算：PH-3 全量规则束(common+platform)必须在预算内完整注入（预算按最大平台束核定，"
               "当前 100K，须 < input_hard_cap）；发生截断必须在日志与提示词中显式列出被丢弃规则 ID，"
               "禁止静默截断；预算不足须先扩容而非裁剪规则", "PH-3", "G3", "PF"),
    "PF-006": ("数据手册类平台规则提炼：以整本数据手册为规则来源的平台（如 E2000），必须先按目录分章、"
               "再分批交 LLM 判定章节是否硬件约束；只保留硬件约束章节；丢弃理由仅限噪声白名单"
               "(revision_history/toc/toc_entry/terminology/ordering/packaging/legal/figure_caption/cover)"
               "且必须逐章留痕于 filter_manifest.json；标题含硬件关键词"
