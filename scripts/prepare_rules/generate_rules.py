...
PROCESS = [
    ...
    {"stage": "PH-1", "name": "网表解析(多EDN全局合并+信号链)", "owner": "hw_prep",
     "gate": "G1", "rules": ["PREP-*", "W0-*"], "batch_boundary": False},
    {"stage": "PH-2", "name": "手册检索", "owner": "hw_search",
     "gate": "G2", "rules": ["G0-*", "RG0-*"], "batch_boundary": True},
    {"stage": "PH-3", "name": "数据预检(Wave0)", "owner": "flow(确定性)",
     "gate": "G3", "rules": ["W0-*", "RW1-*"], "batch_boundary": False},
    {"stage": "PH-4", "name": "深度分析(芯片级并行<=5)", "owner": "hw_analyze",
     "gate": "G4", "rules": ["IC-*", "PO-*", "CN-*", "DR-*", "PE-*", "LS-*", "PB-*", "IF-*"], "batch_boundary": True},
    {"stage": "PH-5", "name": "报告合成(report.json+渲染.md)", "owner": "hw_write",
     "gate": "G5", "rules": ["RF-*", "CT-*"], "batch_boundary": False},
    {"stage": "PH-6", "name": "审计复核", "owner": "hw_auditor",
     "gate": "G6", "rules": ["SA-*", "Q-*"], "batch_boundary": True},
    {"stage": "PH-7", "name": "闭环交付", "owner": "human+flow",
     "gate": "G7", "rules": [], "batch_boundary": True},
]
GATES = {
    "G1": {"name": "prep_validate", "after": "PH-1", "note": "prep JSON schema + 信号链端到端闭合(确定性)"},
    "G2": {"name": "手册缺失确认", "after": "PH-2", "note": "人机 A/B/C；缺失→补/跳过(UNVERIFIED)/排除"},
    "G3": {"name": "数据完整性", "after": "PH-3", "note": "Wave0关键总线单连接/链路闭合(确定性)"},
    "G4": {"name": "g2x_validate", "after": "PH-4", "note": "evidence契约/填充率>=80%/接口覆盖(确定性)"},
...
PATH_MAP = {  # B-01 路径域映射（raw 约定 → 本项目）
    ...
    ".sisyphus/evidence/": "storge/project/<产品>/E_analyze/evidence/",
    "{REFBOOK_DIR}": "storge/refbook",
    "{DOWNLOAD_DIR}": "storge/datasheet",
    "报告输出(项目根)": "storge/project/<产品>/F_report/",
}
