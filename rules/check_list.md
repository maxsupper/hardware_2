...
- **G1**（manual_bom_validate）@ PH-1：manual_index 覆盖全部U* + BOM解析无错误/条目非空/板号可识别
- **G2**（netlist_validate）@ PH-2：netlist_graph完备性：dangling=0/uncovered=0/devices全覆盖/跨板连续
- **G3**（g2x_validate）@ PH-3：evidence契约/填充率>=80%/接口覆盖(确定性)+复核回环<=3轮(计数独立)
- **G4**（报告审核门）@ PH-4：确定性结构校验 + LLM内容审核(规则+要求+内容,有界2轮)
...
| CL-001 | PH-4↔PH-3 回环 request/resolution 为 JSON、有 | v2_design::PH-3::G3 | 0 |
| CL-002 | PH-4 不得读原始 EDN/xlsx，仅读 netlist_graph.jso | v2_design::PH-3::G3 | 0 |
...
| NG-001 | netlist_graph：devices 覆盖全部 (板,位号)，pins 为 | v2_design::PH-2::G2 | 0 |
| NG-002 | links 覆盖每个非串联器件引脚（uncovered=0）；纯芯片间网 sid | v2_design::PH-2::G2 | 0 |
