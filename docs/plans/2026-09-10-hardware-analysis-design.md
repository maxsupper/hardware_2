...
| PH-4 深度分析 | LLM 芯片级并行≤5，**只读 netlist_graph.json**+复核 tracer 判定+回环 | hw_analyze | G4 | netlist_graph | PH-3_深度分析/*.json |
| PH-5 报告合成 | report.json + .md | hw_write | G5 | PH-3_深度分析 | report |
